...
const App = () => {
  ...
  const getFlowChart = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URI}/api/botFlowByuid`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        if (data.data && data.data.new__data !== null) {
          try {
          ...
          // data.data is null or data.data.new__data is null -> Initialize default node
          if (bot_flow_type === "Normal") {
            createDefaultQuestionNode();
          } else {
            createDefaultWebhookNode();
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      if (type === "catlognode") {
        ...
        fetch(`${import.meta.env.VITE_BASE_URI}/api/checkCatalog`, {
          method: 'POST',
          headers: {
