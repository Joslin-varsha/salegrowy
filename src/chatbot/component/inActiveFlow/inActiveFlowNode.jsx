...
function InactiveNode({ data, selected, id }) {
  ...
  // Local state for label & replyMessage
  const [label, setLabel] = useState(data.label || "Inactive Followup Node Created");
  const [replyMessage, setReplyMessage] = useState(data.replyMessage || "");
  const vendor_uid = localStorage.getItem('vendor_uid');
  const bot_flow_uid = localStorage.getItem('bot_flow_uid');

  // Function to replace placeholders if needed
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        ...
        <div className="px-3 py-2 text-left text-white text-xs font-semibold rounded-t-lg bg-gradient-to-r from-blue-500 to-indigo-500 flex justify-between items-center">
          <span className="flex items-center gap-1">
            <Camera size={14} className="opacity-90" /> Inactive Followup Node
          </span>
      ...
      {/* Buttons */}
      {data.button_1 && data.button_type === "reply" && (
        <div className="py-1 relative px-2">
          <button className="rounded-[3px] bg-blue-500 text-white hover:bg-blue-600 p-2 flex text-dark w-full">{data.button_1}</button>
          <Handle
            id="handle-button1"
            type="source"
      ...
      {data.button_2 && data.button_type === "reply" && (
        <div className="py-2 px-2 relative">
          <button className="rounded-[3px] bg-blue-500 text-white hover:bg-blue-600 p-2 flex text-dark w-full">{data.button_2}</button>
          <Handle
            id="handle-button2"
            type="source"
            position={Position.Right}
      ...
      {data.button_3 && data.button_type === "reply" && (
        <div className="py-2 px-2 relative">
          <button className="rounded-[3px] bg-blue-500 text-white hover:bg-blue-600 p-2 flex text-dark w-full">{data.button_3}</button>
          <Handle
            id="handle-button3"
            type="source"
            position={Position.Right}
      ...
      {data.button_type === "list" &&
        data.sections?.map((section) => (
          <div
            key={section.id}
            className="py-2 ml-4 mr-4 mb-4 relative border p-2 rounded bg-gray-50">
            {/* <Handle
