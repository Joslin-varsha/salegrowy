...
function PaymentNode({ data, selected, id }) {
  ...
  const userData =
    data?.userId != null
      ...
      : dataList[0]; // fallback user data

  // Replace placeholders like {name} with actual data values from node data or userData
  const formatLabel = (label) => {
    if (!label) return "";
    return label.replace(/\{(\s*\w+\s*)\}/g, (match, key) => {
      const trimmedKey = key.trim();
      return data?.[trimmedKey] || userData?.[trimmedKey] || match;
    });
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        ...
        <div className="px-3 py-2 text-left text-white text-xs font-semibold rounded-t-lg bg-gradient-to-r from-blue-500 to-indigo-500 flex justify-between items-center">
          <span className="flex items-center gap-1">
            <Camera size={14} className="opacity-90" /> Payment Link Node
          </span>
        ...
        <div className="px-4 py-3 text-xs text-black">
          <div className="py-2">
            ...
            >
              {(!data.label || data.label === "paymentnode") ? "Type your name here..." : data.label}
      ...
      {/* Handles must be direct children of the main wrapper */}
      <Handle
        id="a"
        type="target"
