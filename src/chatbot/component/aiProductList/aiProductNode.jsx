...
function AiProductNode({ data, selected, id, nodeName, setNodeName }) {
  const { deleteElements } = useReactFlow();
  const [inputValue, setInputValue] = useState(nodeName || data?.label || "");

  // Keep inputValue in sync with nodeName/data.label
  useEffect(() => {
    setInputValue(nodeName || data?.label || "");
  }, [nodeName, data?.label]);
      ...
      <div className="flex flex-col">
        ...
        <div className="px-3 py-2 text-left text-white text-xs font-semibold rounded-t-lg bg-gradient-to-r from-blue-500 to-indigo-500 flex justify-between items-center">
          <span className="flex items-center gap-1">
            <Camera size={14} className="opacity-90" /> AI Product Node
          </span>
        ...
        <div className="px-3 py-2 text-xs text-black">
          <div className="py-2 relative">
            ...
            <p
              className={`mt-2 p-2 border rounded bg-gray-50 text-xs ${data.label && data.label.length ? "text-gray-700" : "text-gray-400"
                }`}
            >
              {(!data.label || data.label === "aiProductNode") ? "Type your name here..." : data.label}
            </p>
