...
const AIAgent = () => {
  ...
  const fetchAiStatus = async () => {
    try {
      const response = await axios.post(`${BASE_URI}getWidgetAi`, { vendor_id: vendorId });
...
const KnowledgeBaseContent = ({ setShowRedirectDialog }) => {
  ...
  const fetchInitialStatuses = async () => {
    if (!vendorId) return;
    try {
      const resp = await axios.post(`${BASE_URI}getActivePlatform`, { vendor_id: vendorId });
      if (resp.data.status) {
  ...
  const ShopifyConfig = () => {
    ...
    const fetchToken = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.post(`${BASE_URI}getShopifyToken`, { vendor_id: vendorId });
  ...
  const WooCommerceConfig = () => {
    ...
    const fetchToken = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.post(`${BASE_URI}getWoocommerceToken`, { vendor_id: vendorId });
        if (response.data.status && response.data.data) {
          const data = response.data.data;
          setWooData({
  ...
  const MetaConfig = () => {
    ...
    const fetchToken = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.post(`${BASE_URI}getMetaToken`, { vendor_id: vendorId });
        if (response.data.status && response.data.data) {
          const data = response.data.data;
          setMetaData({
...
const ActionsContent = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.post(`${BASE_URI}get-users`, { vendorId: vendorId });
      if (response.data.status) {
  ...
  const fetchAiBotSettings = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${BASE_URI}aiBotSettingView`, { vendorId: vendorId });
...
const AiStopContactsContent = () => {
    ...
    const fetchStoppedContacts = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.post(`${BASE_URI}aiStopedChats`, { vendorId: vendorId });
